...
void _DD3D_WorldWatcher::_process(double p_delta) {
	set_process(false);
	if (!m_owner) {
		PRINT_WARNING("{0} is an internal DebugDraw3D node. Remove it from your scene to avoid crashes in the future.",
				get_name());
	}
}
...
void _DD3D_WorldWatcher::_notification(int p_what) {
	if ((p_what == NOTIFICATION_EXIT_WORLD || p_what == NOTIFICATION_EXIT_TREE) && m_owner) {
		...
		if (!is_queued_for_deletion()) {
			if (get_parent()) { get_parent()->call_deferred(NAMEOF(remove_child), this); }
			queue_free();
		}
	}
}
...
#pragma region Draw Functions
	...
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_sphere), "position", "radius", "color", "duration"),
			&DebugDraw3D::draw_sphere, 0.5f, Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_sphere_xf), "transform", "color", "duration"),
			&DebugDraw3D::draw_sphere_xf, Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_cylinder), "transform", "color", "duration"), &DebugDraw3D::draw_cylinder,
			Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_cylinder_ab), "a", "b", "radius", "color", "duration"),
			&DebugDraw3D::draw_cylinder_ab, 0.5f, Colors::empty_color, 0);

	ClassDB::bind_method(
			D_METHOD(NAMEOF(draw_box), "position", "rotation", "size", "color", "is_box_centered", "duration"),
			&DebugDraw3D::draw_box, Colors::empty_color, false, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_box_ab), "a", "b", "up", "color", "is_ab_diagonal", "duration"),
			&DebugDraw3D::draw_box_ab, Vector3(0, 1, 0), Colors::empty_color, true, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_box_xf), "transform", "color", "is_box_centered", "duration"),
			&DebugDraw3D::draw_box_xf, Colors::empty_color, true, 0);
	ClassDB::bind_method(
			D_METHOD(NAMEOF(draw_aabb), "aabb", "color", "duration"), &DebugDraw3D::draw_aabb, Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_aabb_ab), "a", "b", "color", "duration"), &DebugDraw3D::draw_aabb_ab,
			Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_line_hit), "start", "end", "hit", "is_hit", "hit_size", "hit_color",
								 "after_hit_color", "duration"),
			&DebugDraw3D::draw_line_hit, 0.25f, Colors::empty_color, Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_line_hit_offset), "start", "end", "is_hit", "unit_offset_of_hit",
								 "hit_size", "hit_color", "after_hit_color", "duration"),
			&DebugDraw3D::draw_line_hit_offset, 0.5f, 0.25f, Colors::empty_color, Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_line), "a", "b", "color", "duration"), &DebugDraw3D::draw_line,
			Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_lines), "lines", "color", "duration"), &DebugDraw3D::draw_lines,
			Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_ray), "origin", "direction", "length", "color", "duration"),
			&DebugDraw3D::draw_ray, Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_line_path), "path", "color", "duration"), &DebugDraw3D::draw_line_path,
			Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_arrowhead), "transform", "color", "duration"),
			&DebugDraw3D::draw_arrowhead, Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_arrow), "a", "b", "color", "arrow_size", "is_absolute_size", "duration"),
			&DebugDraw3D::draw_arrow, Colors::empty_color, 0.5f, false, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_arrow_ray), "origin", "direction", "length", "color", "arrow_size",
								 "is_absolute_size", "duration"),
			&DebugDraw3D::draw_arrow_ray, Colors::empty_color, 0.5f, false, 0);
	ClassDB::bind_method(
			D_METHOD(NAMEOF(draw_arrow_path), "path", "color", "arrow_size", "is_absolute_size", "duration"),
			&DebugDraw3D::draw_arrow_path, Colors::empty_color, 0.75f, true, 0);

	ClassDB::bind_method(
			D_METHOD(NAMEOF(draw_point_path), "path", "type", "size", "points_color", "lines_color", "duration"),
			&DebugDraw3D::draw_point_path, PointType::POINT_TYPE_SQUARE, 0.25f, Colors::empty_color,
			Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_square), "position", "size", "color", "duration"),
			&DebugDraw3D::draw_square, 0.2f, Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_plane), "plane", "color", "anchor_point", "duration"),
			&DebugDraw3D::draw_plane, Colors::empty_color, Vector3_INF, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_points), "points", "type", "size", "color", "duration"),
			&DebugDraw3D::draw_points, PointType::POINT_TYPE_SQUARE, 0.2f, Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_camera_frustum), "camera", "color", "duration"),
			&DebugDraw3D::draw_camera_frustum, Colors::empty_color, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_camera_frustum_planes), "camera_frustum", "color", "duration"),
			&DebugDraw3D::draw_camera_frustum_planes, Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_position), "transform", "color", "duration"), &DebugDraw3D::draw_position,
			Colors::empty_color, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_gizmo), "transform", "color", "is_centered", "duration"),
			&DebugDraw3D::draw_gizmo, Colors::empty_color, false, 0);

	ClassDB::bind_method(D_METHOD(NAMEOF(draw_grid), "origin", "x_size", "y_size", "subdivision", "color",
								 "is_centered", "duration"),
			&DebugDraw3D::draw_grid, Colors::empty_color, true, 0);
	ClassDB::bind_method(D_METHOD(NAMEOF(draw_grid_xf), "transform", "subdivision", "color", "is_centered", "duration"),
			&DebugDraw3D::draw_grid_xf, Colors::empty_color, true, 0);
...
DebugDraw3D::DebugDraw3D() { ASSIGN_SINGLETON(DebugDraw3D); }
...
void DebugDraw3D::init(DebugDrawManager* p_root) {
	...
	DEFINE_SETTING_AND_GET_HINT(real_t def_frustum_scale, root_settings_section + s_default_frustum_scale, 0.5f,
			Variant::FLOAT, PROPERTY_HINT_RANGE, "0,1,0.0001");

	DEFINE_SETTING_AND_GET_HINT(real_t def_thickness, root_settings_section + s_default_thickness, 0.05f,
			Variant::FLOAT, PROPERTY_HINT_RANGE, "0,100,0.0001,or_greater");
	DEFINE_SETTING_AND_GET_HINT(real_t def_brightness, root_settings_section + s_default_center_brightness, 0.8f,
			Variant::FLOAT, PROPERTY_HINT_RANGE, "0,1,0.0001");
	DEFINE_SETTING_AND_GET(real_t def_hd_sphere, root_settings_section + s_default_hd_spheres, false, Variant::BOOL);
	DEFINE_SETTING_AND_GET_HINT(real_t def_plane_size, root_settings_section + s_default_plane_size, 0, Variant::FLOAT,
			PROPERTY_HINT_RANGE, "0,10000,0.001,or_greater");

	DEFINE_SETTING(root_settings_section + s_render_priority, 0, Variant::INT);
	DEFINE_SETTING_HINT(root_settings_section + s_render_mode, 0, Variant::INT, PROPERTY_HINT_ENUM,
			"Default,Forced Transparent,Forced Opaque");
...
void DebugDraw3D::process(double p_delta) {
	...
	for (const auto& p : debug_containers) {
		for (const auto& dgc : p.second) {
			if (dgc) { dgc->update_geometry(p_delta); }
		}
	}
...
void DebugDraw3D::physics_process_start(double p_delta) {
	...
	for (const auto& p : debug_containers) {
		for (const auto& dgc : p.second) {
			if (dgc) { dgc->update_geometry_physics_start(p_delta); }
		}
	}
#endif
}
...
void DebugDraw3D::physics_process_end(double p_delta) {
	...
	for (const auto& p : debug_containers) {
		for (const auto& dgc : p.second) {
			if (dgc) { dgc->update_geometry_physics_end(p_delta); }
		}
	}
...
const std::shared_ptr<DebugDraw3DScopeConfig::Data> DebugDraw3D::scoped_config_for_current_thread() {
	...
	if (it != cached_scoped_configs.cend()) { return it->second; }
...
void DebugDraw3D::_unregister_scoped_config(uint64_t thread_id, uint64_t guard_id) {
	...
	auto res = std::find_if(
			cfgs.rbegin(), cfgs.rend(), [&guard_id](const ScopedPairIdConfig& i) { return i.first == guard_id; });

	if (res != cfgs.rend()) {
		cfgs.erase(--res.base());

		// Update cached value
		if (!cfgs.empty()) { cached_scoped_configs[thread_id] = cfgs.back().second->data; }
		else { cached_scoped_configs[thread_id] = default_scoped_config.ptr()->data; }
	}
}
...
void DebugDraw3D::_clear_scoped_configs() {
	...
	if (orphans)
		PRINT_ERROR("{0} scoped configs weren't freed. Do not save scoped configurations anywhere other than function "
					"bodies.",
				orphans);
}

Node* DebugDraw3D::get_root_node() { return root_node; }
...
#define GEN_MESH(_type, _gen)                                                                                          \
	...
	shared_generated_meshes[(int)_type][i]->surface_set_material(                                                      \
			0, get_material_variant(mat_type, i == 0 ? MeshMaterialVariant::Normal : MeshMaterialVariant::NoDepth));

			// WIREFRAME

			mat_type = MeshMaterialType::Wireframe;
			GEN_MESH(InstanceType::CUBE,
					GeometryGenerator::CreateMeshNative(Mesh::PrimitiveType::PRIMITIVE_LINES,
							GeometryGenerator::CubeVertexes, GeometryGenerator::CubeIndexes));
			GEN_MESH(InstanceType::CUBE_CENTERED,
					GeometryGenerator::CreateMeshNative(Mesh::PrimitiveType::PRIMITIVE_LINES,
							GeometryGenerator::CenteredCubeVertexes, GeometryGenerator::CubeIndexes));
			GEN_MESH(InstanceType::ARROWHEAD,
					GeometryGenerator::CreateMeshNative(Mesh::PrimitiveType::PRIMITIVE_LINES,
							GeometryGenerator::ArrowheadVertexes, GeometryGenerator::ArrowheadIndexes));
			GEN_MESH(InstanceType::POSITION,
					GeometryGenerator::CreateMeshNative(Mesh::PrimitiveType::PRIMITIVE_LINES,
							GeometryGenerator::PositionVertexes, GeometryGenerator::PositionIndexes));
			GEN_MESH(InstanceType::SPHERE,
					p_use_icosphere ? GeometryGenerator::CreateIcosphereLines(0.5f, 1)
									: GeometryGenerator::CreateSphereLines(8, 8, 0.5f, 2));
			GEN_MESH(InstanceType::SPHERE_HD,
					p_use_icosphere_hd ? GeometryGenerator::CreateIcosphereLines(0.5f, 2)
									   : GeometryGenerator::CreateSphereLines(16, 16, 0.5f, 2));
			GEN_MESH(InstanceType::CYLINDER, GeometryGenerator::CreateCylinderLines(16, 1, 1, 2));
			GEN_MESH(InstanceType::CYLINDER_AB,
					GeometryGenerator::RotatedMesh(GeometryGenerator::CreateCylinderLines(16, 1, 1, 2), Vector3_RIGHT,
							Math::deg_to_rad(90.f)));

			// VOLUMETRIC

			mat_type = MeshMaterialType::Extendable;
			GEN_MESH(InstanceType::LINE_VOLUMETRIC,
					GeometryGenerator::ConvertWireframeToVolumetric(
							GeometryGenerator::CreateMeshNative(
									Mesh::PrimitiveType::PRIMITIVE_LINES, GeometryGenerator::LineVertexes),
							p_add_bevel));
			GEN_MESH(InstanceType::CUBE_VOLUMETRIC,
					GeometryGenerator::ConvertWireframeToVolumetric(
							shared_generated_meshes[(int)InstanceType::CUBE][i], p_add_bevel));
			GEN_MESH(InstanceType::CUBE_CENTERED_VOLUMETRIC,
					GeometryGenerator::ConvertWireframeToVolumetric(
							shared_generated_meshes[(int)InstanceType::CUBE_CENTERED][i], p_add_bevel));
			GEN_MESH(InstanceType::ARROWHEAD_VOLUMETRIC,
					GeometryGenerator::CreateVolumetricArrowHead(.25f, 1.f, 1.f, p_add_bevel));
			GEN_MESH(InstanceType::POSITION_VOLUMETRIC,
					GeometryGenerator::ConvertWireframeToVolumetric(
							shared_generated_meshes[(int)InstanceType::POSITION][i], p_add_bevel));
			GEN_MESH(InstanceType::SPHERE_VOLUMETRIC,
					GeometryGenerator::ConvertWireframeToVolumetric(
							shared_generated_meshes[(int)InstanceType::SPHERE][i], false));
			GEN_MESH(InstanceType::SPHERE_HD_VOLUMETRIC,
					GeometryGenerator::ConvertWireframeToVolumetric(
							shared_generated_meshes[(int)InstanceType::SPHERE_HD][i], false));
			GEN_MESH(InstanceType::CYLINDER_VOLUMETRIC,
					GeometryGenerator::ConvertWireframeToVolumetric(
							shared_generated_meshes[(int)InstanceType::CYLINDER][i], false));
			GEN_MESH(InstanceType::CYLINDER_AB_VOLUMETRIC,
					GeometryGenerator::ConvertWireframeToVolumetric(
							shared_generated_meshes[(int)InstanceType::CYLINDER_AB][i], false));

			// SOLID

			mat_type = MeshMaterialType::Billboard;
			GEN_MESH(InstanceType::BILLBOARD_SQUARE,
					GeometryGenerator::CreateMeshNative(Mesh::PrimitiveType::PRIMITIVE_TRIANGLES,
							GeometryGenerator::CenteredSquareVertexes, GeometryGenerator::SquareBackwardsIndexes));

			mat_type = MeshMaterialType::Plane;
			GEN_MESH(InstanceType::PLANE,
					GeometryGenerator::CreateMeshNative(Mesh::PrimitiveType::PRIMITIVE_TRIANGLES,
							GeometryGenerator::CenteredSquareVertexes, GeometryGenerator::SquareIndexes));
#undef GEN_MESH
		}
	}
...
std::shared_ptr<DebugGeometryContainer> DebugDraw3D::get_debug_container(
		const DebugDraw3DScopeConfig::DebugContainerDependent& p_dgcd, const bool p_generate_new_container) {
	ZoneScoped;
	LOCK_GUARD(datalock);

	if (!p_dgcd.viewport) { return nullptr; }
	...
	if (!p_generate_new_container) { return nullptr; }
	...
	call_deferred(NAMEOF(_register_viewport_world_deferred),
			_get_root_world_viewport(p_dgcd.viewport)->get_instance_id(), vp_world_id);
...
Viewport* DebugDraw3D::_get_root_world_viewport(Viewport* p_vp) {
	...
	if (!parent_vp || parent_vp == p_vp) return p_vp;

	if (p_vp->find_world_3d() == parent_vp->find_world_3d()) { return _get_root_world_viewport(parent_vp); }
...
void DebugDraw3D::_remove_debug_container(const uint64_t& p_world_id) {
	...
	if (const auto& dgc = debug_containers.find(p_world_id); dgc != debug_containers.end()) {
		...
		for (const auto& p : viewport_to_world_cache) {
			if (p.second.world_id == p_world_id) { viewport_to_remove.push_back(p.first); }
		}
...
Ref<DebugDraw3DScopeConfig> DebugDraw3D::new_scoped_config() {
	...
	Ref<DebugDraw3DScopeConfig> res(
			memnew(DebugDraw3DScopeConfig(thread, create_counter, scoped_config_for_current_thread(), unreg_func)));
...
Ref<DebugDraw3DScopeConfig> DebugDraw3D::scoped_config() { return default_scoped_config; }
...
#define LOAD_SHADER(mat, source)                                                                                       \
	...
	bool fog_disabled = Utils::is_current_godot_version_in_range(4, 2)
			? (bool)PS()->get_setting(root_settings_section + s_render_fog_disabled)
			: false;

	for (int variant = 0; variant < (int)MeshMaterialVariant::MAX; variant++) {
		String prefix = "";
		if (variant == (int)MeshMaterialVariant::NoDepth) { prefix += "#define NO_DEPTH\n"; }

		switch (render_mode) {
			case 0: break;
			case 1: prefix += "#define FORCED_TRANSPARENT\n"; break;
			case 2: prefix += "#define FORCED_OPAQUE\n"; break;
		}

		if (Utils::is_current_godot_version_in_range(4, 2)) {
			if (fog_disabled) { prefix += "#define FOG_DISABLED\n"; }
		}

		LOAD_SHADER(mesh_shaders[(int)MeshMaterialType::Wireframe][variant],
				prefix + DD3DResources::src_resources_wireframe_unshaded_gdshader);
		LOAD_SHADER(mesh_shaders[(int)MeshMaterialType::Billboard][variant],
				prefix + DD3DResources::src_resources_billboard_unshaded_gdshader);
		LOAD_SHADER(mesh_shaders[(int)MeshMaterialType::Plane][variant],
				prefix + DD3DResources::src_resources_plane_unshaded_gdshader);
		LOAD_SHADER(mesh_shaders[(int)MeshMaterialType::Extendable][variant],
				prefix + DD3DResources::src_resources_extendable_meshes_gdshader);
	}
#undef LOAD_SHADER
#endif
}
...
std::vector<SubViewport*> DebugDraw3D::get_custom_editor_viewports() { return custom_editor_viewports; }
...
Color DebugDraw3D::get_empty_color() const { return Colors::empty_color; }
...
bool DebugDraw3D::is_debug_enabled() const { return debug_enabled; }

void DebugDraw3D::set_config(Ref<DebugDraw3DConfig> cfg) {
	if (cfg.is_valid()) { config = cfg; }
	else {
...
Ref<DebugDraw3DConfig> DebugDraw3D::get_config() const { return config; }
...
#define GET_PROC_TYPE()                                                                                                \
	(Engine::get_singleton()->is_in_physics_frame() ? ProcessType::PHYSICS_PROCESS : ProcessType::PROCESS)
...
Vector3 DebugDraw3D::get_up_vector(const Vector3& p_dir) {
	if (Math::is_equal_approx(p_dir.x, 0)) {
		if (Math::is_equal_approx(p_dir.z, 0)) return Vector3_FORWARD;
		return Vector3_UP;
	}
	else if (Math::is_equal_approx(p_dir.y, 0)) { return p_dir.normalized().cross(Vector3_UP); }
	else if (Math::is_equal_approx(p_dir.z, 0)) { return Vector3_UP; }

	return Vector3_UP;
}

void DebugDraw3D::add_or_update_line_with_thickness(real_t p_exp_time, std::unique_ptr<Vector3[]> p_lines,
		const size_t p_line_count, const Color& p_col, const std::function<void(DelayedRendererLine*)> p_custom_upd) {
	ZoneScoped;
	...
	if (!scfg->thickness) {
		dgc->geometry_pool.add_or_update_line(
				scfg, p_exp_time, GET_PROC_TYPE(), std::move(p_lines), p_line_count, p_col);
	}
	else {
		for (int i = 0; i < p_line_count; i += 2) {
			...
			dgc->geometry_pool.add_or_update_instance(scfg, InstanceType::LINE_VOLUMETRIC, p_exp_time, GET_PROC_TYPE(),
					Transform3D(Basis().looking_at(center, get_up_vector(center)).scaled(VEC3_ONE(len)), a), // slow
					p_col, SphereBounds(a + center, len * .5f));
		}
	}
}
...
void DebugDraw3D::draw_sphere_base(const Transform3D& transform, const Color& color, const real_t& duration) {
	...
	dgc->geometry_pool.add_or_update_instance(scfg, ConvertableInstanceType::SPHERE, duration, GET_PROC_TYPE(),
			transform, IS_DEFAULT_COLOR(color) ? Colors::chartreuse : color,
			SphereBounds(transform.origin, MathUtils::get_max_basis_length(transform.basis) * 0.5f));
}

void DebugDraw3D::draw_sphere(
		const Vector3& position, const real_t& radius, const Color& color, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
...
void DebugDraw3D::draw_cylinder(const Transform3D& transform, const Color& color, const real_t& duration) {
	...
	dgc->geometry_pool.add_or_update_instance(scfg, ConvertableInstanceType::CYLINDER, duration, GET_PROC_TYPE(),
			transform, IS_DEFAULT_COLOR(color) ? Colors::forest_green : color,
			SphereBounds(transform.origin,
					MathUtils::get_max_basis_length(transform.basis) * MathUtils::CylinderRadiusForSphere));
}

void DebugDraw3D::draw_cylinder_ab(
		const Vector3& a, const Vector3& b, const real_t& radius, const Color& color, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
	...
	Transform3D t = Transform3D(
			Basis().looking_at(half_center, up).scaled_local(Vector3(radius, radius, len)), a + half_center);

	LOCK_GUARD(datalock);
	GET_SCOPED_CFG_AND_DGC();

	dgc->geometry_pool.add_or_update_instance(scfg, ConvertableInstanceType::CYLINDER_AB, duration, GET_PROC_TYPE(), t,
...
void DebugDraw3D::draw_box(const Vector3& position, const Quaternion& rotation, const Vector3& size, const Color& color,
...
void DebugDraw3D::draw_box_ab(const Vector3& a, const Vector3& b, const Vector3& up, const Color& color,
		const bool& is_ab_diagonal, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
	...
	if (is_ab_diagonal) {
		...
		SphereBounds sb(
				t.origin + half_center_orig, MathUtils::get_max_basis_length(t.basis) * MathUtils::CubeRadiusForSphere);

		LOCK_GUARD(datalock);
		GET_SCOPED_CFG_AND_DGC();

		dgc->geometry_pool.add_or_update_instance(scfg, ConvertableInstanceType::CUBE, duration, GET_PROC_TYPE(), t,
				IS_DEFAULT_COLOR(color) ? Colors::forest_green : color, sb);
	}
	else {
...
void DebugDraw3D::draw_box_xf(
		const Transform3D& transform, const Color& color, const bool& is_box_centered, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();

	/// It's possible to use less space to contain box, but this method works better in more cases
	SphereBounds sb(
			transform.origin, MathUtils::get_max_basis_length(transform.basis) * MathUtils::CubeRadiusForSphere);
	...
	dgc->geometry_pool.add_or_update_instance(scfg,
			is_box_centered ? ConvertableInstanceType::CUBE_CENTERED : ConvertableInstanceType::CUBE, duration,
			GET_PROC_TYPE(), transform, IS_DEFAULT_COLOR(color) ? Colors::forest_green : color, sb);
}
...
void DebugDraw3D::draw_line_hit(const Vector3& start, const Vector3& end, const Vector3& hit, const bool& is_hit,
		const real_t& hit_size, const Color& hit_color, const Color& after_hit_color, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();

	LOCK_GUARD(datalock);
	if (is_hit) {
		add_or_update_line_with_thickness(duration, std::unique_ptr<Vector3[]>(new Vector3[2]{ start, hit }), 2,
				IS_DEFAULT_COLOR(hit_color) ? config->get_line_hit_color() : hit_color);
		add_or_update_line_with_thickness(duration, std::unique_ptr<Vector3[]>(new Vector3[2]{ hit, end }), 2,
				IS_DEFAULT_COLOR(after_hit_color) ? config->get_line_after_hit_color() : after_hit_color);

		GET_SCOPED_CFG_AND_DGC();

		dgc->geometry_pool.add_or_update_instance(scfg, InstanceType::BILLBOARD_SQUARE, duration, GET_PROC_TYPE(),
				Transform3D(Basis().scaled(VEC3_ONE(hit_size)), hit),
				IS_DEFAULT_COLOR(hit_color) ? config->get_line_hit_color() : hit_color,
				SphereBounds(hit, MathUtils::CubeRadiusForSphere * hit_size), &Colors::empty_color);
	}
	else {
		add_or_update_line_with_thickness(duration, std::unique_ptr<Vector3[]>(new Vector3[2]{ start, end }), 2,
				IS_DEFAULT_COLOR(hit_color) ? config->get_line_hit_color() : hit_color);
	}
}

void DebugDraw3D::draw_line_hit_offset(const Vector3& start, const Vector3& end, const bool& is_hit,
		const real_t& unit_offset_of_hit, const real_t& hit_size, const Color& hit_color, const Color& after_hit_color,
		const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();

	if (is_hit && unit_offset_of_hit >= 0 && unit_offset_of_hit <= 1) {
		draw_line_hit(start, end, ((end - start).normalized() * start.distance_to(end) * unit_offset_of_hit + start),
				is_hit, hit_size, hit_color, after_hit_color, duration);
	}
	else { draw_line_hit(start, end, {}, is_hit, hit_size, hit_color, after_hit_color, duration); }
}
...
void DebugDraw3D::draw_line(const Vector3& a, const Vector3& b, const Color& color, const real_t& duration) {
	...
	add_or_update_line_with_thickness(duration, std::unique_ptr<Vector3[]>(new Vector3[2]{ a, b }), 2,
...
void DebugDraw3D::draw_ray(const Vector3& origin, const Vector3& direction, const real_t& length, const Color& color,
		const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();

	add_or_update_line_with_thickness(duration,
			std::unique_ptr<Vector3[]>(new Vector3[2]{ origin, origin + direction * length }), 2,
			IS_DEFAULT_COLOR(color) ? Colors::red : color);
}
...
void DebugDraw3D::draw_line_path(const PackedVector3Array& path, const Color& color, const real_t& duration) {
	...
	if (path.size() < 2) {
		if (path.size() == 1) { PRINT_ERROR("Line path must contains at least 2 points."); }
		return;
	}
...
void DebugDraw3D::create_arrow(const Vector3& p_a, const Vector3& p_b, const Color& p_color, const real_t& p_arrow_size,
		const bool& p_is_absolute_size, const real_t& p_duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
	...
	LOCK_GUARD(datalock);
	dgc->geometry_pool.add_or_update_instance(scfg, ConvertableInstanceType::ARROWHEAD, p_duration, GET_PROC_TYPE(), t,
...
void DebugDraw3D::draw_arrowhead(const Transform3D& transform, const Color& color, const real_t& duration) {
	...
	dgc->geometry_pool.add_or_update_instance(scfg, ConvertableInstanceType::ARROWHEAD, duration, GET_PROC_TYPE(),
			transform, IS_DEFAULT_COLOR(color) ? Colors::light_green : color,
			SphereBounds(transform.origin + transform.basis.get_column(2) * 0.5f,
					MathUtils::ArrowRadiusForSphere * MathUtils::get_max_basis_length(transform.basis)));
}

void DebugDraw3D::draw_arrow(const Vector3& a, const Vector3& b, const Color& color, const real_t& arrow_size,
...
void DebugDraw3D::draw_arrow_ray(const Vector3& origin, const Vector3& direction, const real_t& length,
		const Color& color, const real_t& arrow_size, const bool& is_absolute_size, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
	draw_arrow(origin, origin + direction * length, color, arrow_size, is_absolute_size, duration);
}

void DebugDraw3D::draw_arrow_path(const PackedVector3Array& path, const Color& color, const real_t& arrow_size,
		const bool& is_absolute_size, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
...
void DebugDraw3D::draw_point_path(const PackedVector3Array& path, const PointType type, const real_t& size,
		const Color& points_color, const Color& lines_color, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
...
void DebugDraw3D::draw_square(const Vector3& position, const real_t& size, const Color& color, const real_t& duration) {
	...
	dgc->geometry_pool.add_or_update_instance(scfg, InstanceType::BILLBOARD_SQUARE, duration, GET_PROC_TYPE(), t,
			IS_DEFAULT_COLOR(color) ? Colors::red : color,
			SphereBounds(position, MathUtils::CubeRadiusForSphere * size), &Colors::empty_color);
}

void DebugDraw3D::draw_plane(
		const Plane& plane, const Color& color, const Vector3& anchor_point, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
	...
	Vector3 center_pos =
			plane.project(anchor_point == Vector3_INF ? (cam ? cam->get_global_position() : Vector3()) : anchor_point);
	...
	Color custom_col = Color::from_hsv(front_color.get_h(), Math::clamp(front_color.get_s() - 0.25f, 0.f, 1.f),
			Math::clamp(front_color.get_v() - 0.25f, 0.f, 1.f), front_color.a);

	dgc->geometry_pool.add_or_update_instance(scfg, InstanceType::PLANE, duration, GET_PROC_TYPE(), t, front_color,
			SphereBounds(center_pos, MathUtils::CubeRadiusForSphere * plane_size), &custom_col);
}

void DebugDraw3D::draw_points(const PackedVector3Array& points, const PointType type, const real_t& size,
		const Color& color, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
	...
	for (int i = 0; i < points.size(); i++) {
		switch (type) {
			case PointType::POINT_TYPE_SQUARE: draw_square(points[i], size, color, duration); break;
			case PointType::POINT_TYPE_SPHERE: draw_sphere(points[i], size, color, duration); break;
		}
	}
}
...
void DebugDraw3D::draw_position(const Transform3D& transform, const Color& color, const real_t& duration) {
	...
	dgc->geometry_pool.add_or_update_instance(scfg, ConvertableInstanceType::POSITION, duration, GET_PROC_TYPE(),
			transform, IS_DEFAULT_COLOR(color) ? Colors::crimson : color,
			SphereBounds(transform.origin,
					MathUtils::get_max_basis_length(transform.basis) * MathUtils::AxisRadiusForSphere));
}

void DebugDraw3D::draw_gizmo(
		const Transform3D& transform, const Color& color, const bool& is_centered, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
...
void DebugDraw3D::draw_grid(const Vector3& origin, const Vector3& x_size, const Vector3& y_size,
		const Vector2i& subdivision, const Color& color, const bool& is_centered, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
	draw_grid_xf(Transform3D(Basis(x_size, y_size.cross(x_size).normalized(), y_size), origin), subdivision, color,
			is_centered, duration);
}

void DebugDraw3D::draw_grid_xf(const Transform3D& transform, const Vector2i& p_subdivision, const Color& color,
		const bool& is_centered, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
	...
	subdivision =
			Vector2i(Math::clamp(subdivision.x, 1, MAX_SUBDIVISIONS), Math::clamp(subdivision.y, 1, MAX_SUBDIVISIONS));
...
void DebugDraw3D::draw_camera_frustum_planes_c(
		const std::array<Plane, 6>& planes, const Color& color, const real_t& duration) {
	ZoneScoped;
	CHECK_BEFORE_CALL();
...
void DebugDraw3D::draw_camera_frustum_planes(const Array& camera_frustum, const Color& color, const real_t& duration) {
	...
	if (camera_frustum.size() == 6) {
		...
		PRINT_ERROR(
				"Camera frustum requires an array of 6 planes. Recieved " + String::num_int64(camera_frustum.size()));
	}
